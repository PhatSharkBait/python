...
def sl_mirroClusterDefine(clusterHandle):
    ...
    clusDeformer = cmds.listConnections(clusterHandle, type="cluster")[0]
    #clusDeformer = listConnections((clusterHandle + ".worldMatrix[0]"),type ="cluster",d = 1)


    clusSets = listSets(o = clusDeformer)
    #clusSets = listConnections(clusDeformer[0], type = "objectSet" )

    components = cmds.cluster(clusDeformer, query=True, cmp=True)
    ...
    for vertex in components:
        pair = [vertex]       
        valueW = percent(clusDeformer,vertex,q = 1,v = 1)
